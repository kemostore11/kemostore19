...
window.addEventListener("load",()=>{
    ...
    setTimeout(()=>{
        ...
        hint.innerHTML = `
    ${t("theme-hint")}
    <button id="hideThemeHint">×</button>
`;
...
document.getElementById("hideThemeHint").onclick = ()=>{

    hint.remove();

    localStorage.setItem("hideThemeHint","true");

};
        // حفظ أنه ظهر في الزيارة الحالية
        sessionStorage.setItem("themeHintShown","true");

if(localStorage.getItem("hideThemeHint") === "true") return;
        setTimeout(()=>{
